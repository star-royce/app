...
layui.define(function (exports) {
  ...
  *****************************************************************************/
    ...
    (function ($) {
        ...
        $.i18n.properties = function (settings) {
            ...
            files.forEach(function (file) {
               ...
               // defaultFileName = settings.path + file + '.txt';
                // 2. with language code (eg, Messages_pt.properties)
               // var shortCode = settings.language.substring(0, 2);
                //shortFileName = settings.path + file + '_' + shortCode + '.txt';
                // 3. with language code and country code (eg, Messages_pt_BR.properties)
                if (settings.language.length >= 5) {
                    var longCode = settings.language.substring(0, 5);
                    longFileName = settings.path + file + '_' + longCode + '.txt';
    ...
    $.i18n.loadProperties = function (type) {
        if (!type) {
    ...
    function i18n_Text() {
        ...
        var nodes = document.querySelectorAll('[i18n_Text]');
        ...
        for (var j = 0; j < nodes.length; j++) {
            ...
            if (node.nodeType === 1) {
                ...
                var Text = node.getAttribute("i18n_Text");

                if (Text) {
                    if (typeof ($.i18n.map[Text]) != "undefined")
                    {
                        node.innerText = $.i18n.map[Text];
                    }
    ...
    function i18n_Attr() {
        ...
        var nodes = document.querySelectorAll('[i18n_Attr]');
        ...
        for (var j = 0; j < nodes.length; j++) {
            ...
            if (node.nodeType === 1) {
                ...
                var attr_config = node.getAttribute("i18n_Attr");

                if (attr_config) {

                    var attrs = attr_config.split(';');

                    for (var i = 0; i < attrs.length; i++) {

                        if (attrs[i]) {
                            var attr = attrs[i].split(':');
                            if (typeof ($.i18n.map[attr[1]]) != "undefined") {
                               node.setAttribute(attr[0], $.i18n.map[attr[1]]);
                            }
